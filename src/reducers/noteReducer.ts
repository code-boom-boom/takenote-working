...
const noteReducer = (state = initialState, action): NoteState => {
  switch (action.type) {
    ...
    case ActionType.DELETE_NOTE:
      const deleteNoteIndex = state.data.findIndex(note => note.id === action.payload)
      let newActiveNoteId: string

      if (deleteNoteIndex === 0) {
        if (state.data.find((note, i) => i === 1)) {
          newActiveNoteId = state.data[deleteNoteIndex + 1].id
        } else {
          newActiveNoteId = ''
        }
      } else if (state.data[deleteNoteIndex - 1]) {
        newActiveNoteId = state.data[deleteNoteIndex - 1].id
      } else {
        newActiveNoteId = ''
      }
